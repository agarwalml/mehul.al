...
const Project = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<article class="${"grid grid-cols-6 gap-10 bg-lime-100 border border-lime-300 rounded-lg p-10"}"><aside class="${"flex items-center"}"><img class="${"max-w-full"}"${add_attribute("src", project.logo, 0)}${add_attribute("alt", project.name, 0)}></aside>
  <div class="${"col-span-5 space-y-5"}"><header class="${"space-y-1"}"><h2 class="${"text-xl text-lime-900 font-bold"}">${escape(project.name)}</h2>
      ${project.subtitle ? `<h3 class="${"text-base text-lime-600"}">${escape(project.subtitle)}</h3>` : ``}</header>
...
<section id="${"projects"}" class="${"max-w-3xl w-11/12 mx-auto my-10 space-y-10"}"><header><h1 class="${"text-2xl text-lime-900 font-bold"}">Projects</h1></header>
