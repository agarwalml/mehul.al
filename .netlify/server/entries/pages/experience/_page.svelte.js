...
const Experience = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<article class="${"text-gray-700 bg-lime-100 border border-lime-300 rounded-lg p-10"}"><div class="${"space-y-5"}"><header class="${"space-y-1"}"><div class="${"flex items-baseline justify-between gap-5"}"><h2 class="${"text-xl text-lime-900 font-bold"}">${escape(experience.role)}</h2>
        <p class="${"text-sm text-right"}">${escape(experience.dates)}</p></div>
      <div class="${"flex items-baseline justify-between gap-5"}"><h3 class="${"text-lg text-lime-600"}">${escape(experience.name)}</h3>
...
<section id="${"experience"}" class="${"max-w-3xl w-11/12 mx-auto my-10 space-y-10"}"><header><h1 class="${"text-2xl text-lime-900 font-bold"}">Experience</h1></header>
